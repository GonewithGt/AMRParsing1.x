...
def eval(parsed,gold):
    file_handle0 = []
    file_handle0.append(open(parsed))
    file_handle0.append(open(gold))
    fh = tuple(file_handle0)
    ...
    sent_num = 1
    while True:
        cur_amr1 = get_amr_line(fh[0])
        cur_amr2 = get_amr_line(fh[1])
        if cur_amr1 == "" and cur_amr2 == "":
            break
        if (cur_amr1 == ""):
            print >> sys.stderr, "Error: File 1 has less AMRs than file 2"
